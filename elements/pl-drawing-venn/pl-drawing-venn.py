...
def validate_correct_answers(correct_answers, labels):
    ...
    for i in range(len(correct_answers)):
        ...
        if not set(ans) <= valid_symbols:
            raise Exception(
                f'Invalid characters contained in provided region: "{original_answer}"'
            )
...
def prepare(element_html: str, data: pl.QuestionData) -> None:
    ...
    gradable = pl.get_boolean_attrib(element, "gradable", GRADABLE_DEFAULT)
    correct_ans = pl.get_string_attrib(
        element, "correct-answer", CORRECT_ANSWER_DEFAULT
    )
    circle_radius = pl.get_integer_attrib(
    ...
    if circle_radius * 2 >= width or circle_radius * 2 >= height:
        raise Exception(
            "Default circle radius must be smaller than the height and width of the canvas."
        )
    if gradable:
    ...
    for child in circle_elems:
        ...
        if (show_object or not hide_answer_panel) and (x1 is None or y1 is None):
            raise Exception(
                "Circles to be placed in the Canvas or shown in the answer panel must have x1 and y1 attributes specified."
            )
        ...
        if (x1 < 0 or x1 + circle_radius > width) or (
            y1 < 0 or y1 + circle_radius > height
        ):
    ...
    include_sample = not pl.get_boolean_attrib(
...
def render(element_html: str, data: pl.QuestionData) -> str:
    ...
    name = pl.get_string_attrib(element, "answers-name", "")
    gradable = pl.get_boolean_attrib(element, "gradable", GRADABLE_DEFAULT)
    show_help_text = not pl.get_boolean_attrib(
        element, "hide-help-text", HIDE_HELP_TEXT_DEFAULT
    )
    show_score_badge = not pl.get_boolean_attrib(
    ...
    html_params = {
        ...
        or (data["panel"] == "submission" and gradable)
        or (data["panel"] == "answer" and gradable and not hide_answer_panel),
        "submission": data["panel"] == "submission",
        "help_text": data["panel"] == "question" and gradable and show_help_text,
        "show_score_badge": (
            data["panel"] == "question" or data["panel"] == "submission"
        )
        and gradable
        and show_score_badge,
        "graded": graded,
        "is_100": score == 1 if score is not None else False,
        "is_0": score == 0 if score is not None else False,
        "parse_error": format_errors
